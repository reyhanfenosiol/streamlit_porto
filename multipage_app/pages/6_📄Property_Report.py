...
st.set_page_config(
    page_title="Property Report Analyst",
    page_icon="📄",
    layout="wide"
)

st.title("📄 AI Property Report Analyst")
st.markdown("AI Assistant for analyzing your PDF content briefly.")
st.markdown("**The PDF refers to** the Outlook of Property Market in Indonesia in 2026.")
st.caption("**Source:** https://aesia.kemenkeu.go.id/static/uploads/Advisory-Pulse-Januari-2026.pdf")
...
def search_pdf_trends(query: str) -> str:
    """
    Find market opportunity and macroeconomic trends from the PDF Property Opportunity 2026.
...
- Give concise macro and market opportunity
...
default_query = """Di 2 tahun ke depan pada pasar properti di Indonesia, 
                ...
                for tool_call in ai_message.tool_calls:
                    with steps_placeholder:
                        with st.expander(f"🔹 STEP {step_counter}: Using Tool `{tool_call['name']}`", expanded=True):
                            ...
                            # 1. Tampilkan Raw Output / Data Mentah menggunakan st.code
                            st.markdown("**📄 Observation / Output (Raw):**")
                            st.code(observation, language="text")
                            
                            # 2. Proses pembersihan teks agar ukuran font di st.info kembali normal
                            # Mengonversi ke string (jika output berbentuk objek)
                            obs_string = str(observation)
                            
                            # Regex ini menghapus tanda #, ##, ### dst di awal baris/paragraf
                            clean_observation = re.sub(r'^\s*#+\s*', '', obs_string, flags=re.MULTILINE)
                            
                            # 3. Tampilkan teks yang sudah bersih di st.info (Ukuran teks akan normal)
                            # st.markdown("**ℹ️ Cleaned View:**")
                            # st.info(clean_observation)
                    
                    # Masukkan output tool ke riwayat pesan
                    messages.append(ToolMessage(content=clean_observation, tool_call_id=tool_call['id']))
                    step_counter += 1
